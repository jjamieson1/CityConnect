...
| G·1-025 Attachments incl. camera capture, mandatory malware scan | **Have** *(CIT-26)* | Scanning happens **before** storage: uploads land in quarantine, stream to clamd, and are promoted into the served tree only on a clean verdict. Infected files are deleted; unscanned ones stay quarantined and are never served, while the request is still accepted. Portal upload with `capture="environment"` so a phone opens the camera. Nothing can record `"skipped"` any more — and a deployment with **no** scanner now refuses attachments outright rather than quarantining them for ever, on the staff route as well as the public one. The catalogue tells the portal whether a file can be taken, so the control is never offered where the endpoint would refuse it. |
